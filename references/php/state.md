...
[← PHP 选择指南](README.md) · [Skill 入口](../../SKILL.md) · [可运行源码](../../examples/php/state/main.php)
...
具体状态返回下一态，Context 保存状态接口；状态变化不依赖重复布尔开关。持久工作流必须另外定义状态存储、转换原子性和幂等事件。
...
```sh
php examples/php/state/main.php
```

期望标准输出：`OK state`。任何内嵌断言失败都应导致非零退出；Python 不要使用 `-O` 禁用断言，Swift 不要使用 `-Ounchecked`。

**当前源码状态：已通过运行与内嵌断言。** [完整验证记录](../../docs/VERIFICATION.md)。源码 SHA-256：`c15a0d9b2b39d744649854d634b6da01bb2016f207b5bd13290b03caf28b018e`。
