...
[← PHP 选择指南](README.md) · [Skill 入口](../../SKILL.md) · [可运行源码](../../examples/php/iterator/main.php)
...
IteratorAggregate 每次创建独立 Generator，使 foreach 正常工作。生成器是有状态的一次遍历对象，不能把同一已结束生成器当可重用集合。
...
```sh
php examples/php/iterator/main.php
```

期望标准输出：`OK iterator`。任何内嵌断言失败都应导致非零退出；Python 不要使用 `-O` 禁用断言，Swift 不要使用 `-Ounchecked`。

**当前源码状态：已通过运行与内嵌断言。** [完整验证记录](../../docs/VERIFICATION.md)。源码 SHA-256：`f5644a90e02cf0df0b7ad2460d6e94f1933793e1b02243c009122b7efa30aedf`。
