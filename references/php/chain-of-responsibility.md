# PHP · 责任链（Chain of Responsibility）

[← PHP 选择指南](README.md) · [Skill 入口](../../SKILL.md) · [可运行源码](../../examples/php/chain-of-responsibility/main.php)
...
Closure 是合法属性类型，callable 不能直接用于普通 PHP 属性类型声明。空安全调用与 ?? 定义链尾成功，&& 实现失败短路。
...
```sh
php examples/php/chain-of-responsibility/main.php
```

期望标准输出：`OK chain-of-responsibility`。任何内嵌断言失败都应导致非零退出；Python 不要使用 `-O` 禁用断言，Swift 不要使用 `-Ounchecked`。

**当前源码状态：已通过运行与内嵌断言。** [完整验证记录](../../docs/VERIFICATION.md)。源码 SHA-256：`478cb1a9869ce63e3c3680769b8040c36f92a6455ebe41f72f3b263b61ec1a8b`。
