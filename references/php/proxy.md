...
[← PHP 选择指南](README.md) · [Skill 入口](../../SKILL.md) · [可运行源码](../../examples/php/proxy/main.php)
...
空对象字段实现按需创建，完整 Image 接口仍对外可见。例子未使用 PHP 新版本的原生 lazy object API；不依赖超出 8.2 基线的新特性。
...
```sh
php examples/php/proxy/main.php
```

期望标准输出：`OK proxy`。任何内嵌断言失败都应导致非零退出；Python 不要使用 `-O` 禁用断言，Swift 不要使用 `-Ounchecked`。

**当前源码状态：已通过运行与内嵌断言。** [完整验证记录](../../docs/VERIFICATION.md)。源码 SHA-256：`503b42210d0fa1a60884a3d6c8186395f4fbf785541c1c6242d4bdaef8a82b8e`。
