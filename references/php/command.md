...
[← PHP 选择指南](README.md) · [Skill 入口](../../SKILL.md) · [可运行源码](../../examples/php/command/main.php)
...
对象句柄让命令操作同一接收者；显式生命周期防止重复操作。内存前值恢复不等于跨请求持久撤销，也不支持与其他命令的交错修改。
...
```sh
php examples/php/command/main.php
```

期望标准输出：`OK command`。任何内嵌断言失败都应导致非零退出；Python 不要使用 `-O` 禁用断言，Swift 不要使用 `-Ounchecked`。

**当前源码状态：已通过运行与内嵌断言。** [完整验证记录](../../docs/VERIFICATION.md)。源码 SHA-256：`5ee9699355ded289459ae411ff6bfd216eee6c67b7832096d1983b1ae1764b53`。
