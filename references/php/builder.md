...
[← PHP 选择指南](README.md) · [Skill 入口](../../SKILL.md) · [可运行源码](../../examples/php/builder/main.php)
...
数组字符串值的写时复制使两份配置隔离，readonly 固定成品属性；若数组元素是可变对象，仍会共享对象句柄，不能宣称自动深不可变。
...
```sh
php examples/php/builder/main.php
```

期望标准输出：`OK builder`。任何内嵌断言失败都应导致非零退出；Python 不要使用 `-O` 禁用断言，Swift 不要使用 `-Ounchecked`。

**当前源码状态：已通过运行与内嵌断言。** [完整验证记录](../../docs/VERIFICATION.md)。源码 SHA-256：`49f14ebcb2809f0994ee978368be57e2a312effb81c0c4992d9abd038786f6fe`。
