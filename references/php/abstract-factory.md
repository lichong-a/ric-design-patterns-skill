# PHP · 抽象工厂（Abstract Factory）

[← PHP 选择指南](README.md) · [Skill 入口](../../SKILL.md) · [可运行源码](../../examples/php/abstract-factory/main.php)
...
PHP 8.2 readonly class 固定产品字段，构造器属性提升减轻样板。两个产品接口是族结构，产品主题由工厂保证而非类型系统证明。
...
```sh
php examples/php/abstract-factory/main.php
```

期望标准输出：`OK abstract-factory`。任何内嵌断言失败都应导致非零退出；Python 不要使用 `-O` 禁用断言，Swift 不要使用 `-Ounchecked`。

**当前源码状态：已通过运行与内嵌断言。** [完整验证记录](../../docs/VERIFICATION.md)。源码 SHA-256：`d0c7b9771e3a00599e44eaacfa5a157ae34b0889eb763ba70c23edcd57cd722c`。
