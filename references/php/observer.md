...
[← PHP 选择指南](README.md) · [Skill 入口](../../SKILL.md) · [可运行源码](../../examples/php/observer/main.php)
...
数组快照固定当前轮回调列表，按令牌退订。同步异常直接向上传播；不是消息队列，不提供跨请求投递、重试或消费者隔离。
...
```sh
php examples/php/observer/main.php
```

期望标准输出：`OK observer`。任何内嵌断言失败都应导致非零退出；Python 不要使用 `-O` 禁用断言，Swift 不要使用 `-Ounchecked`。

**当前源码状态：已通过运行与内嵌断言。** [完整验证记录](../../docs/VERIFICATION.md)。源码 SHA-256：`e751abb97f5fe41a33872dee293c0b5193ed340ea7836df8b53c0ca4eceb227b`。
