...
[← PHP 选择指南](README.md) · [Skill 入口](../../SKILL.md) · [可运行源码](../../examples/php/memento/main.php)
...
快照通过私有身份令牌授权源发器读取内部文本，管理者只保存对象。源发器禁克隆，避免复制令牌产生意外共享身份；反射/持久序列化另需约束。
...
```sh
php examples/php/memento/main.php
```

期望标准输出：`OK memento`。任何内嵌断言失败都应导致非零退出；Python 不要使用 `-O` 禁用断言，Swift 不要使用 `-Ounchecked`。

**当前源码状态：已通过运行与内嵌断言。** [完整验证记录](../../docs/VERIFICATION.md)。源码 SHA-256：`2feee1d0156d91b9b82ec3ac23eea54ba800be5a79c18f44029ee1a7f121f95c`。
