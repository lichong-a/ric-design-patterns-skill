...
[← PHP 选择指南](README.md) · [Skill 入口](../../SKILL.md) · [可运行源码](../../examples/php/visitor/main.php)
...
不同 visit 方法名避免 PHP 不支持按参数类型重载的限制；节点主动进行第二次分派。新增节点需要修改访问者契约，不能同时宣称两轴扩展都无成本。
...
```sh
php examples/php/visitor/main.php
```

期望标准输出：`OK visitor`。任何内嵌断言失败都应导致非零退出；Python 不要使用 `-O` 禁用断言，Swift 不要使用 `-Ounchecked`。

**当前源码状态：已通过运行与内嵌断言。** [完整验证记录](../../docs/VERIFICATION.md)。源码 SHA-256：`f2d3f99823b80e39e8d470c3b5506a9ca55e73ab548a3c195ef070df03f57fd7`。
