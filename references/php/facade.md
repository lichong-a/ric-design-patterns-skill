...
[← PHP 选择指南](README.md) · [Skill 入口](../../SKILL.md) · [可运行源码](../../examples/php/facade/main.php)
...
构造注入保持依赖可见，外观不吞异常。本例没有数据库/远程支付，因此不把一次方法调用当成业务原子性证明。
...
```sh
php examples/php/facade/main.php
```

期望标准输出：`OK facade`。任何内嵌断言失败都应导致非零退出；Python 不要使用 `-O` 禁用断言，Swift 不要使用 `-Ounchecked`。

**当前源码状态：已通过运行与内嵌断言。** [完整验证记录](../../docs/VERIFICATION.md)。源码 SHA-256：`a7a886a88ceb9b6e9074c95bb177c166482ed69414978b03eab0dfbb79fc9c13`。
