...
[← PHP 选择指南](README.md) · [Skill 入口](../../SKILL.md) · [可运行源码](../../examples/php/interpreter/main.php)
...
显式 AST 解释小型表达式，校验环境值类型而非调用 eval。整型加法超范围会受 PHP 数值转换/返回类型约束影响，生产实现应定义溢出规则。
...
```sh
php examples/php/interpreter/main.php
```

期望标准输出：`OK interpreter`。任何内嵌断言失败都应导致非零退出；Python 不要使用 `-O` 禁用断言，Swift 不要使用 `-Ounchecked`。

**当前源码状态：已通过运行与内嵌断言。** [完整验证记录](../../docs/VERIFICATION.md)。源码 SHA-256：`830fabe4feaed83d374792ca6fcc0e8d4f64f62d7e4d5e0878711774363b0e05`。
