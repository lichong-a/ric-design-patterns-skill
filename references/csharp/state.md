...
[← C# 选择指南](README.md) · [Skill 入口](../../SKILL.md) · [可运行源码](../../examples/csharp/state/Program.cs)
...
| State | IGateState：coin / enter 等状态行为 |
...
状态实现事件协议并返回下一态，Context 不重复维护布尔状态。小状态集合可用 enum+switch expression；持久转换还需并发冲突与事务策略。
...
```sh
dotnet run --project examples/csharp/state/Example.csproj --configuration Release
```
