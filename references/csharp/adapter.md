...
[← C# 选择指南](README.md) · [Skill 入口](../../SKILL.md) · [可运行源码](../../examples/csharp/adapter/Program.cs)
...
构造器校验 null，避免 nullable 注解被误认为运行时保证；组合旧服务显式转换单位。readonly 字段只限制引用重新赋值。
...
```sh
dotnet run --project examples/csharp/adapter/Example.csproj --configuration Release
```

期望标准输出：`OK adapter`。任何内嵌断言失败都应导致非零退出；Python 不要使用 `-O` 禁用断言，Swift 不要使用 `-Ounchecked`。

**当前源码状态：待验证（无匹配当前源码哈希的运行记录）。** [完整验证记录](../../docs/VERIFICATION.md)。源码 SHA-256：`d484ff27bd35a18470cd85f725bda1177dcbfff85df955b3e656285630812b06`。
