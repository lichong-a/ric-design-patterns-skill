...
[← C# 选择指南](README.md) · [Skill 入口](../../SKILL.md) · [可运行源码](../../examples/csharp/facade/Program.cs)
...
构造器注入保持依赖可见；异常直接传播，不伪造成功。生产异步外观通常还要传递 CancellationToken，且不会自动获得跨服务事务。
...
```sh
dotnet run --project examples/csharp/facade/Example.csproj --configuration Release
```

期望标准输出：`OK facade`。任何内嵌断言失败都应导致非零退出；Python 不要使用 `-O` 禁用断言，Swift 不要使用 `-Ounchecked`。

**当前源码状态：待验证（无匹配当前源码哈希的运行记录）。** [完整验证记录](../../docs/VERIFICATION.md)。源码 SHA-256：`b341de4cd9f7d992dcd80b3b9e6cc17b27a939f1c3a15348fab05e7fe664035e`。
