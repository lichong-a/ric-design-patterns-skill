...
[← C# 选择指南](README.md) · [Skill 入口](../../SKILL.md) · [可运行源码](../../examples/csharp/chain-of-responsibility/Program.cs)
...
Func 委托表达处理者，短路操作符定义验证链行为。ASP.NET 中间件常有额外的双向调用/异步语义，不能与本例逐项验证简单等同。
...
```sh
dotnet run --project examples/csharp/chain-of-responsibility/Example.csproj --configuration Release
```

期望标准输出：`OK chain-of-responsibility`。任何内嵌断言失败都应导致非零退出；Python 不要使用 `-O` 禁用断言，Swift 不要使用 `-Ounchecked`。

**当前源码状态：待验证（无匹配当前源码哈希的运行记录）。** [完整验证记录](../../docs/VERIFICATION.md)。源码 SHA-256：`8110abcc293ae0238554d8d2879991d61907c0937b1c619bb5b5e9c4c098f32e`。
