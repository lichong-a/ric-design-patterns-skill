...
[← C# 选择指南](README.md) · [Skill 入口](../../SKILL.md) · [可运行源码](../../examples/csharp/observer/Program.cs)
...
令牌管理与回调快照定义当前轮语义；生产 .NET 还可选 event 或 IObservable<T>/IObserver<T>。此同步总线不是线程安全/异常隔离的响应式流。
...
```sh
dotnet run --project examples/csharp/observer/Example.csproj --configuration Release
```

期望标准输出：`OK observer`。任何内嵌断言失败都应导致非零退出；Python 不要使用 `-O` 禁用断言，Swift 不要使用 `-Ounchecked`。

**当前源码状态：待验证（无匹配当前源码哈希的运行记录）。** [完整验证记录](../../docs/VERIFICATION.md)。源码 SHA-256：`48e15a0ccec71b81c7d8c2f04d1c0ac1e207a3259584274a126c945b55650cc8`。
