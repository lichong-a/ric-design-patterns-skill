...
[← C# 选择指南](README.md) · [Skill 入口](../../SKILL.md) · [可运行源码](../../examples/csharp/flyweight/Program.cs)
...
record 默认提供值相等，测试用 ReferenceEquals 区分真正共享实例。Dictionary 是单线程池；换 ConcurrentDictionary 也不代表工厂委托永远只执行一次。
...
```sh
dotnet run --project examples/csharp/flyweight/Example.csproj --configuration Release
```
