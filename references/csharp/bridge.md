...
[← C# 选择指南](README.md) · [Skill 入口](../../SKILL.md) · [可运行源码](../../examples/csharp/bridge/Program.cs)
...
|---|---|
| Abstraction | Notice：持有 IChannel 并组织消息 |
| RefinedAbstraction | UrgentNotice：改变消息形式 |
| Implementor | IChannel：发送接口 |
...
抽象轴使用 virtual，通道轴通过接口字段组合。实际通道若是 IDisposable，应明确由组合对象还是 DI 容器拥有和释放，不能重复 Dispose。
...
```sh
dotnet run --project examples/csharp/bridge/Example.csproj --configuration Release
```
