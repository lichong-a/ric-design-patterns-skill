...
[← C# 选择指南](README.md) · [Skill 入口](../../SKILL.md) · [可运行源码](../../examples/csharp/mediator/Program.cs)
...
event 限制外部触发，用具名处理函数方便 -= 解绑；Dialog 实现 IDisposable 清理注册。重复 Dispose 在此例无副作用，真实资源需要明确所有权。
...
```sh
dotnet run --project examples/csharp/mediator/Example.csproj --configuration Release
```
