...
[← C# 选择指南](README.md) · [Skill 入口](../../SKILL.md) · [可运行源码](../../examples/csharp/prototype/Program.cs)
...
显式 DeepCopy 定义嵌套列表的复制深度；string 不可变可共享。MemberwiseClone 与 record with 通常不会深复制被引用对象，不能冒称通用深克隆。
...
```sh
dotnet run --project examples/csharp/prototype/Example.csproj --configuration Release
```
