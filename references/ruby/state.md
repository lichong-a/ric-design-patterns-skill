...
[← Ruby 选择指南](README.md) · [Skill 入口](../../SKILL.md) · [可运行源码](../../examples/ruby/state/main.rb)
...
|---|---|
| Context | Gate：持有当前状态并委派事件（未显式声明的接口角色由方法约定承担） |
| State | GateState：coin / enter 等状态行为（未显式声明的接口角色由方法约定承担） |
| ConcreteState | Locked / Unlocked：各自规则与转换（未显式声明的接口角色由方法约定承担） |
...
鸭子类型状态实现同一事件协议，状态自身决定下一态；Context 只转发。小型固定状态可用符号加 case，更复杂的持久工作流不能仅靠对象切换。
...
```sh
ruby examples/ruby/state/main.rb
```

期望标准输出：`OK state`。任何内嵌断言失败都应导致非零退出；Python 不要使用 `-O` 禁用断言，Swift 不要使用 `-Ounchecked`。

**当前源码状态：已通过运行与内嵌断言。** [完整验证记录](../../docs/VERIFICATION.md)。源码 SHA-256：`19b4c0943cb8f4962188017cdcd294452accbc4b2344c6304b27022b7ca6c7bb`。
