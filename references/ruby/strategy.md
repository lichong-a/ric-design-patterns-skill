...
[← Ruby 选择指南](README.md) · [Skill 入口](../../SKILL.md) · [可运行源码](../../examples/ruby/strategy/main.rb)
...
lambda 直接表达单操作策略；多方法、资源或可观测状态才更适合策略对象。Proc 与 lambda 的参数/return 语义不同，不应随意互换。
...
```sh
ruby examples/ruby/strategy/main.rb
```

期望标准输出：`OK strategy`。任何内嵌断言失败都应导致非零退出；Python 不要使用 `-O` 禁用断言，Swift 不要使用 `-Ounchecked`。

**当前源码状态：已通过运行与内嵌断言。** [完整验证记录](../../docs/VERIFICATION.md)。源码 SHA-256：`0b68e1489f936c3e238bda7159b1ac484089f89ac7837791d04c1ae9f641669b`。
