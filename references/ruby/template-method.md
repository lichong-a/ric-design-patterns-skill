# Ruby · 模板方法（Template Method）

[← Ruby 选择指南](README.md) · [Skill 入口](../../SKILL.md) · [可运行源码](../../examples/ruby/template-method/main.rb)
...
稳定 run 调用受保护步骤，Ruby 不原生强制 final，需要测试约束。组合式 callable pipeline 是替代方案，别为一两个步骤过度继承。
...
```sh
ruby examples/ruby/template-method/main.rb
```

期望标准输出：`OK template-method`。任何内嵌断言失败都应导致非零退出；Python 不要使用 `-O` 禁用断言，Swift 不要使用 `-Ounchecked`。

**当前源码状态：已通过运行与内嵌断言。** [完整验证记录](../../docs/VERIFICATION.md)。源码 SHA-256：`395aa447b6afbe736d894af5c73cd82279aba27988c8ea7697588649fef6c7ba`。
