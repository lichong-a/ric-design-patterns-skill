# Ruby · 责任链（Chain of Responsibility）

[← Ruby 选择指南](README.md) · [Skill 入口](../../SKILL.md) · [可运行源码](../../examples/ruby/chain-of-responsibility/main.rb)
...
lambda/Proc 表达处理动作，显式后继保持责任传递。这个验证链要求所有经过规则都为真；不同业务应明确是首个处理即停还是全链验证。
...
```sh
ruby examples/ruby/chain-of-responsibility/main.rb
```

期望标准输出：`OK chain-of-responsibility`。任何内嵌断言失败都应导致非零退出；Python 不要使用 `-O` 禁用断言，Swift 不要使用 `-Ounchecked`。

**当前源码状态：已通过运行与内嵌断言。** [完整验证记录](../../docs/VERIFICATION.md)。源码 SHA-256：`55ca09cb11dcebe72ab7e2932aa186e8dd344f2bd973a2305e420869575515af`。
