...
[← Ruby 选择指南](README.md) · [Skill 入口](../../SKILL.md) · [可运行源码](../../examples/ruby/proxy/main.rb)
...
|---|---|
| Subject | Image：read 操作（未显式声明的接口角色由方法约定承担） |
| RealSubject | RealImage：实际载入图像的模拟主体（未显式声明的接口角色由方法约定承担） |
| Proxy | LazyImage：首次访问才创建并缓存主体（未显式声明的接口角色由方法约定承担） |
...
惰性字段保留 read 协议，同时控制首次创建。不能把 MRI 的某些执行行为视为所有 Ruby 实现的线程安全保证；多线程需要同步。
...
```sh
ruby examples/ruby/proxy/main.rb
```

期望标准输出：`OK proxy`。任何内嵌断言失败都应导致非零退出；Python 不要使用 `-O` 禁用断言，Swift 不要使用 `-Ounchecked`。

**当前源码状态：已通过运行与内嵌断言。** [完整验证记录](../../docs/VERIFICATION.md)。源码 SHA-256：`9c3aa38a94b9a82aaf0ddf5ef1cfc7dabad0981e0c32f6ca8c173a85084e3671`。
