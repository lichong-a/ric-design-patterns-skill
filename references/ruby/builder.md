# Ruby · 生成器（建造者）（Builder）

[← Ruby 选择指南](README.md) · [Skill 入口](../../SKILL.md) · [可运行源码](../../examples/ruby/builder/main.rb)
...
链式方法返回 self；对选定的字符串、数组和成品逐层复制/冻结。freeze 是浅操作，此例不能推广为任意对象图自动深冻结。
...
```sh
ruby examples/ruby/builder/main.rb
```

期望标准输出：`OK builder`。任何内嵌断言失败都应导致非零退出；Python 不要使用 `-O` 禁用断言，Swift 不要使用 `-Ounchecked`。

**当前源码状态：已通过运行与内嵌断言。** [完整验证记录](../../docs/VERIFICATION.md)。源码 SHA-256：`33f3f9adc6be887ed12668d0fbb17730233d5d412cb1f21accce76dd74a54343`。
