# Ruby · 抽象工厂（Abstract Factory）

[← Ruby 选择指南](README.md) · [Skill 入口](../../SKILL.md) · [可运行源码](../../examples/ruby/abstract-factory/main.rb)
...
工厂协议由 button/checkbox 方法约定而非空接口类构成；行为契约需要共享测试，不能以 respond_to? 证明产品族语义相容。
...
```sh
ruby examples/ruby/abstract-factory/main.rb
```

期望标准输出：`OK abstract-factory`。任何内嵌断言失败都应导致非零退出；Python 不要使用 `-O` 禁用断言，Swift 不要使用 `-Ounchecked`。

**当前源码状态：已通过运行与内嵌断言。** [完整验证记录](../../docs/VERIFICATION.md)。源码 SHA-256：`e6e39f7f40c708e75cc907d2b18e79fc375094a82bc87e05efed6efd4f4738cc`。
