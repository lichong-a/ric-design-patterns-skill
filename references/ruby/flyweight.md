...
[← Ruby 选择指南](README.md) · [Skill 入口](../../SKILL.md) · [可运行源码](../../examples/ruby/flyweight/main.rb)
...
|---|---|
| Flyweight | Style：共享且只读的字体状态（未显式声明的接口角色由方法约定承担） |
| Factory | StylePool：按字体键复用 Style（未显式声明的接口角色由方法约定承担） |
| Context | Glyph：字符/位置以及对 Style 的引用（未显式声明的接口角色由方法约定承担） |
...
equal? 检查共享身份，freeze 同时施加到字体字符串与 Style。Hash + ||= 不构成通用线程安全的单次创建协议；长期缓存需约束大小。
...
```sh
ruby examples/ruby/flyweight/main.rb
```

期望标准输出：`OK flyweight`。任何内嵌断言失败都应导致非零退出；Python 不要使用 `-O` 禁用断言，Swift 不要使用 `-Ounchecked`。

**当前源码状态：已通过运行与内嵌断言。** [完整验证记录](../../docs/VERIFICATION.md)。源码 SHA-256：`64c0bc042b6cc1fff54dc14162cf8c9b19373be42563213d84d46c553796573b`。
