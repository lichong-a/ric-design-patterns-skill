...
[← Ruby 选择指南](README.md) · [Skill 入口](../../SKILL.md) · [可运行源码](../../examples/ruby/mediator/main.rb)
...
block/lambda 将控件通知汇聚到 Dialog；各控件不引用彼此。闭包会保留中介者，外部事件源与长期回调需要显式取消订阅。
...
```sh
ruby examples/ruby/mediator/main.rb
```

期望标准输出：`OK mediator`。任何内嵌断言失败都应导致非零退出；Python 不要使用 `-O` 禁用断言，Swift 不要使用 `-Ounchecked`。

**当前源码状态：已通过运行与内嵌断言。** [完整验证记录](../../docs/VERIFICATION.md)。源码 SHA-256：`8d3c4329fca1c153664b1385101a5e6a9493334fd95d235bfbd812f9005903ba`。
