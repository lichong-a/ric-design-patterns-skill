# Ruby · 解释器（Interpreter）

[← Ruby 选择指南](README.md) · [Skill 入口](../../SKILL.md) · [可运行源码](../../examples/ruby/interpreter/main.rb)
...
Hash#fetch 对缺失变量显式失败；节点遵循 evaluate 协议，不调用 Kernel#eval。动态操作数仍需领域类型约束；整数能力不等于有资源预算或沙箱。
...
```sh
ruby examples/ruby/interpreter/main.rb
```

期望标准输出：`OK interpreter`。任何内嵌断言失败都应导致非零退出；Python 不要使用 `-O` 禁用断言，Swift 不要使用 `-Ounchecked`。

**当前源码状态：已通过运行与内嵌断言。** [完整验证记录](../../docs/VERIFICATION.md)。源码 SHA-256：`da70b627f1be6601da3343f9197d2c441b8cd2228fed6fd15ae29a7a9514364c`。
