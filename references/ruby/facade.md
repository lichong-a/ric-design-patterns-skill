...
[← Ruby 选择指南](README.md) · [Skill 入口](../../SKILL.md) · [可运行源码](../../examples/ruby/facade/main.rb)
...
|---|---|
| Facade | Checkout：协调 place 操作（未显式声明的接口角色由方法约定承担） |
| Subsystem | Inventory / Receipts：独立子系统（未显式声明的接口角色由方法约定承担） |
| Client | 只依赖 Checkout 的用例接口（未显式声明的接口角色由方法约定承担） |
...
构造注入让协调流程易于替换测试；外观未吞异常。真实 ActiveRecord/外部服务事务边界需要单独实现，不能由外观模式自动获得。
...
```sh
ruby examples/ruby/facade/main.rb
```

期望标准输出：`OK facade`。任何内嵌断言失败都应导致非零退出；Python 不要使用 `-O` 禁用断言，Swift 不要使用 `-Ounchecked`。

**当前源码状态：已通过运行与内嵌断言。** [完整验证记录](../../docs/VERIFICATION.md)。源码 SHA-256：`09588b1e3309f6e328be41f6bee9c3c1c250ff29777da4ffee118069218c7cab`。
