...
[← Ruby 选择指南](README.md) · [Skill 入口](../../SKILL.md) · [可运行源码](../../examples/ruby/singleton/main.rb)
...
使用标准库 Singleton 而非自行手写脆弱初始化。它约束常见构造/复制路径，不使后续共享可变数据自动线程安全，也不跨进程共享。
...
```sh
ruby examples/ruby/singleton/main.rb
```

期望标准输出：`OK singleton`。任何内嵌断言失败都应导致非零退出；Python 不要使用 `-O` 禁用断言，Swift 不要使用 `-Ounchecked`。

**当前源码状态：已通过运行与内嵌断言。** [完整验证记录](../../docs/VERIFICATION.md)。源码 SHA-256：`261ecc26eb642f07bac4b1e02a61cc878f4234854b4dbdc70a4b0e182a1f98f9`。
