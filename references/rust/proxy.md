...
[← Rust 选择指南](README.md) · [Skill 入口](../../SKILL.md) · [可运行源码](../../examples/rust/proxy/main.rs)
...
Option 表达延迟状态，&mut self 要求独占访问，避免同一对象的无同步写入。内部 expect 仅断言上一行建立的不变量；真实加载失败应使用 Result 而非 panic。
...
**当前源码状态：待验证（无匹配当前源码哈希的运行记录）。** [完整验证记录](../../docs/VERIFICATION.md)。源码 SHA-256：`dd580a5e8e8fcfa4e6027dac63d67250b2615543c9c8e01ccd3f37d47745419f`。
