# Rust · 解释器（Interpreter）

[← Rust 选择指南](README.md) · [Skill 入口](../../SKILL.md) · [可运行源码](../../examples/rust/interpreter/main.rs)
...
拥有的 Box AST 加借用环境，Result 区分变量缺失与溢出。封闭语法可用递归 enum；这里不包含解析器、深度预算或安全脚本执行。
