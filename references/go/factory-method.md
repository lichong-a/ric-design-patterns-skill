# Go · 工厂方法（Factory Method）

[← Go 选择指南](README.md) · [Skill 入口](../../SKILL.md) · [可运行源码](../../examples/go/factory-method/main.go)

**分类：创建型** · **代码目标：Go 1.20 语法/API 目标；示例不依赖 1.23 range 函数**
...
| Creator | Creator / Publish：发布流程调用 make |
...
Go 不支持类继承；用 Creator 接口的动态 Make 与独立稳定工作流 Publish 保留创建扩展点。这是工厂方法意图的组合式移植，不把普通 switch 构造器冒充经典继承结构。
...
```sh
go run examples/go/factory-method/main.go
```

期望标准输出：`OK factory-method`。任何内嵌断言失败都应导致非零退出；Python 不要使用 `-O` 禁用断言，Swift 不要使用 `-Ounchecked`。

**当前源码状态：已通过编译/运行与内嵌断言。** [完整验证记录](../../docs/VERIFICATION.md)。源码 SHA-256：`ed19915d411f1762eed6149893d1881ca77e702f2dfff156bdcf3e54bb023a2b`。
...
- [参考站语言示例抽样](https://refactoringguru.cn/design-patterns/factory-method/go/example)
