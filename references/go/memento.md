...
[← Go 选择指南](README.md) · [Skill 入口](../../SKILL.md) · [可运行源码](../../examples/go/memento/main.go)
...
未导出字段封装包外 API，不是安全沙箱。身份令牌故意使用非零大小类型，不能依赖不同零大小对象指针一定不同；Editor 应按指针使用而非复制身份。
...
```sh
go run examples/go/memento/main.go
```

期望标准输出：`OK memento`。任何内嵌断言失败都应导致非零退出；Python 不要使用 `-O` 禁用断言，Swift 不要使用 `-Ounchecked`。

**当前源码状态：已通过编译/运行与内嵌断言。** [完整验证记录](../../docs/VERIFICATION.md)。源码 SHA-256：`a0a88955130e6646f0e9753ca5c58d5fc446efeaef1354f1f0fdc79905455794`。
