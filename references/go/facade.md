...
[← Go 选择指南](README.md) · [Skill 入口](../../SKILL.md) · [可运行源码](../../examples/go/facade/main.go)
...
外观显式返回 error，并在失败处短路，不隐藏子系统错误。零值子系统可用仅是示例设计；不等于真实业务具备事务与幂等。
...
```sh
go run examples/go/facade/main.go
```

期望标准输出：`OK facade`。任何内嵌断言失败都应导致非零退出；Python 不要使用 `-O` 禁用断言，Swift 不要使用 `-Ounchecked`。

**当前源码状态：已通过编译/运行与内嵌断言。** [完整验证记录](../../docs/VERIFICATION.md)。源码 SHA-256：`e768e3e91511c3cbdba707083e3a88dd122ad9d76d1a871d9c17833af89a6103`。
