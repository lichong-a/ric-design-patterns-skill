...
[← Go 选择指南](README.md) · [Skill 入口](../../SKILL.md) · [可运行源码](../../examples/go/bridge/main.go)
...
嵌入只复用成员/方法，不是虚方法继承；实现轴通过 Channel 接口组合，通知轴显式实现 Deliver。例子要求非 nil 通道。
...
```sh
go run examples/go/bridge/main.go
```

期望标准输出：`OK bridge`。任何内嵌断言失败都应导致非零退出；Python 不要使用 `-O` 禁用断言，Swift 不要使用 `-Ounchecked`。

**当前源码状态：已通过编译/运行与内嵌断言。** [完整验证记录](../../docs/VERIFICATION.md)。源码 SHA-256：`5daa4cbc49659c6dc1688f6f3dd866ee30fffaf4c703ae73aea9ff1a4f61627c`。
