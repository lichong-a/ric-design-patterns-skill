# Go · 抽象工厂（Abstract Factory）

[← Go 选择指南](README.md) · [Skill 入口](../../SKILL.md) · [可运行源码](../../examples/go/abstract-factory/main.go)
...
小接口隐式满足，无须 implements。产品族的选择由工厂对象决定；用不同接口区分产品种类，主题通过值字段表达。
...
```sh
go run examples/go/abstract-factory/main.go
```

期望标准输出：`OK abstract-factory`。任何内嵌断言失败都应导致非零退出；Python 不要使用 `-O` 禁用断言，Swift 不要使用 `-Ounchecked`。

**当前源码状态：已通过编译/运行与内嵌断言。** [完整验证记录](../../docs/VERIFICATION.md)。源码 SHA-256：`d41b42fea23d64ddc7dbb4e72b62b34d586d37e6cc3f7224ebd4b298d9eb905f`。
