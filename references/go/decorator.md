...
[← Go 选择指南](README.md) · [Skill 入口](../../SKILL.md) · [可运行源码](../../examples/go/decorator/main.go)
...
小接口加显式转发即可堆叠，无需基础装饰类。真实 io 包装器应同时考虑错误传播、Close 所有权和额外接口是否需要转发。
...
```sh
go run examples/go/decorator/main.go
```

期望标准输出：`OK decorator`。任何内嵌断言失败都应导致非零退出；Python 不要使用 `-O` 禁用断言，Swift 不要使用 `-Ounchecked`。

**当前源码状态：已通过编译/运行与内嵌断言。** [完整验证记录](../../docs/VERIFICATION.md)。源码 SHA-256：`8b2cef93d23203f9828082681b2fabf227f9d40bd02324b811ef4daa629c1e4a`。
