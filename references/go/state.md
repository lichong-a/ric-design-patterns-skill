...
[← Go 选择指南](README.md) · [Skill 入口](../../SKILL.md) · [可运行源码](../../examples/go/state/main.go)
...
小型值状态通过接口返回下一态，Context 持有状态而不维护重复标志。状态集合固定且很小时，枚举加 switch 可能比多个状态类型更简单。
...
```sh
go run examples/go/state/main.go
```

期望标准输出：`OK state`。任何内嵌断言失败都应导致非零退出；Python 不要使用 `-O` 禁用断言，Swift 不要使用 `-Ounchecked`。

**当前源码状态：已通过编译/运行与内嵌断言。** [完整验证记录](../../docs/VERIFICATION.md)。源码 SHA-256：`6a93c6025bbefd62055c6ae437feaa9eb15b0a7ce6b920181c575c351458175d`。
