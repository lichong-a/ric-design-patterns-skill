...
[← Go 选择指南](README.md) · [Skill 入口](../../SKILL.md) · [可运行源码](../../examples/go/singleton/main.go)
...
sync.Once 提供初始化同步和可见性；字段不导出只限制包外访问，包内仍可构造。Once 不使后续可变字段自动线程安全，跨进程也不唯一。
...
```sh
go run examples/go/singleton/main.go
```

期望标准输出：`OK singleton`。任何内嵌断言失败都应导致非零退出；Python 不要使用 `-O` 禁用断言，Swift 不要使用 `-Ounchecked`。

**当前源码状态：已通过编译/运行与内嵌断言。** [完整验证记录](../../docs/VERIFICATION.md)。源码 SHA-256：`d6a30f4864c9fef7eb3c5eae483661667038b5ebf6989ac15ef1d7896bb14bf6`。
