...
[← Go 选择指南](README.md) · [Skill 入口](../../SKILL.md) · [可运行源码](../../examples/go/composite/main.go)
...
叶子与容器隐式实现 Item；值组合适合教学树。实际构建 API 应拒绝 nil/负值/环；interface 内含 typed nil 时不能只比较接口等于 nil。
...
```sh
go run examples/go/composite/main.go
```

期望标准输出：`OK composite`。任何内嵌断言失败都应导致非零退出；Python 不要使用 `-O` 禁用断言，Swift 不要使用 `-Ounchecked`。

**当前源码状态：已通过编译/运行与内嵌断言。** [完整验证记录](../../docs/VERIFICATION.md)。源码 SHA-256：`5f2be0cb1507ca640a797eb6dc8ae44cb0dea9df73e97a8d296c21bb4e5d0c9d`。
