...
[← Go 选择指南](README.md) · [Skill 入口](../../SKILL.md) · [可运行源码](../../examples/go/proxy/main.go)
...
指针接收者修改延迟加载状态；这不是并发安全代理。初始化可能失败时，不应机械使用不能自动重试的 sync.Once。
...
```sh
go run examples/go/proxy/main.go
```

期望标准输出：`OK proxy`。任何内嵌断言失败都应导致非零退出；Python 不要使用 `-O` 禁用断言，Swift 不要使用 `-Ounchecked`。

**当前源码状态：已通过编译/运行与内嵌断言。** [完整验证记录](../../docs/VERIFICATION.md)。源码 SHA-256：`0face92180b3a087608900f623d53d0cec03a8891086fd485d98ec54ae6e32eb`。
