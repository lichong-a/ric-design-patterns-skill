...
[← Go 选择指南](README.md) · [Skill 入口](../../SKILL.md) · [可运行源码](../../examples/go/adapter/main.go)
...
结构体组合旧 API 并隐式满足新接口；先转 float64 再除法，防止整数截断。边界转换仍需单独处理错误与单位约定。
...
```sh
go run examples/go/adapter/main.go
```

期望标准输出：`OK adapter`。任何内嵌断言失败都应导致非零退出；Python 不要使用 `-O` 禁用断言，Swift 不要使用 `-Ounchecked`。

**当前源码状态：已通过编译/运行与内嵌断言。** [完整验证记录](../../docs/VERIFICATION.md)。源码 SHA-256：`3fe4c824a27f7a918229e5f0388fe3245f3abd21c93653982bafdcadd6528897`。
