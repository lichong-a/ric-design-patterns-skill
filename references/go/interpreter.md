...
[← Go 选择指南](README.md) · [Skill 入口](../../SKILL.md) · [可运行源码](../../examples/go/interpreter/main.go)
...
接口表达 AST 节点，map 的逗号 ok 区分缺失变量与合法零值。没有使用 eval，也未实现解析器、溢出检测和递归预算。
...
```sh
go run examples/go/interpreter/main.go
```

期望标准输出：`OK interpreter`。任何内嵌断言失败都应导致非零退出；Python 不要使用 `-O` 禁用断言，Swift 不要使用 `-Ounchecked`。

**当前源码状态：已通过编译/运行与内嵌断言。** [完整验证记录](../../docs/VERIFICATION.md)。源码 SHA-256：`8f6bd4cd47cf01637ab9b02459e529471781a5f6e4a96d2852c28f62faa19a44`。
