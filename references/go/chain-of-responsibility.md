# Go · 责任链（Chain of Responsibility）

[← Go 选择指南](README.md) · [Skill 入口](../../SKILL.md) · [可运行源码](../../examples/go/chain-of-responsibility/main.go)
...
函数值实现规则，指针连接后继，短路操作符终止传递。验证链是职责链的一种变体；长链、环和上下文取消需要工程约束。
...
```sh
go run examples/go/chain-of-responsibility/main.go
```

期望标准输出：`OK chain-of-responsibility`。任何内嵌断言失败都应导致非零退出；Python 不要使用 `-O` 禁用断言，Swift 不要使用 `-Ounchecked`。

**当前源码状态：已通过编译/运行与内嵌断言。** [完整验证记录](../../docs/VERIFICATION.md)。源码 SHA-256：`a4b2d9c47857ffe8f401f3fdb4929f447798b90986f6d6bdaa1fdc30ad49f4b2`。
