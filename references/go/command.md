...
[← Go 选择指南](README.md) · [Skill 入口](../../SKILL.md) · [可运行源码](../../examples/go/command/main.go)
...
命令持有接收者指针并返回 error，测试约束执行/撤销阶段。快照撤销仅覆盖无交错修改；跨 goroutine 使用须定义同步和版本冲突处理。
...
```sh
go run examples/go/command/main.go
```

期望标准输出：`OK command`。任何内嵌断言失败都应导致非零退出；Python 不要使用 `-O` 禁用断言，Swift 不要使用 `-Ounchecked`。

**当前源码状态：已通过编译/运行与内嵌断言。** [完整验证记录](../../docs/VERIFICATION.md)。源码 SHA-256：`ff184bfccf82393b71b2b9d27bb4ae0d335548e6758bad1676f69a14edf58e62`。
