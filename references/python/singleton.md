...
[← Python 选择指南](README.md) · [Skill 入口](../../SKILL.md) · [可运行源码](../../examples/python/singleton/main.py)
...
采用模块级共享实例，不宣称禁止调用 _Settings 构造器。唯一性限定同一个已加载模块；reload、多个解释器或进程可能产生不同实例。
...
```sh
python3 examples/python/singleton/main.py
```

期望标准输出：`OK singleton`。任何内嵌断言失败都应导致非零退出；Python 不要使用 `-O` 禁用断言，Swift 不要使用 `-Ounchecked`。

**当前源码状态：已通过运行与内嵌断言。** [完整验证记录](../../docs/VERIFICATION.md)。源码 SHA-256：`191062cac60d1f7230f3642257ecbd4f38399ffc3adb7c76f6e4433c21cf92bb`。
