...
[← Python 选择指南](README.md) · [Skill 入口](../../SKILL.md) · [可运行源码](../../examples/python/memento/main.py)
...
下划线表达模块内部约定，并非安全隔离；历史保存者只保存/交回 Snapshot。不可变字符串无需深复制，owner token 防止误恢复其他对象。
...
```sh
python3 examples/python/memento/main.py
```

期望标准输出：`OK memento`。任何内嵌断言失败都应导致非零退出；Python 不要使用 `-O` 禁用断言，Swift 不要使用 `-Ounchecked`。

**当前源码状态：已通过运行与内嵌断言。** [完整验证记录](../../docs/VERIFICATION.md)。源码 SHA-256：`f7c6a4cfc1ee46258546147ef68fbe0ae2f3beccc9691740d2ca799dc450b9b0`。
