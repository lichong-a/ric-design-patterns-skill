...
[← Python 选择指南](README.md) · [Skill 入口](../../SKILL.md) · [可运行源码](../../examples/python/visitor/main.py)
...
元素显式回调 visitor 的类型专属方法，而不是 isinstance 链；Protocol 配合延迟注解表达双向引用。新增节点时须更新访问者契约。
...
```sh
python3 examples/python/visitor/main.py
```

期望标准输出：`OK visitor`。任何内嵌断言失败都应导致非零退出；Python 不要使用 `-O` 禁用断言，Swift 不要使用 `-Ounchecked`。

**当前源码状态：已通过运行与内嵌断言。** [完整验证记录](../../docs/VERIFICATION.md)。源码 SHA-256：`9a80de9627634afdbf1066353c691f5f53ffc269e65161e8982a4a40586b3161`。
