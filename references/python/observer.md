...
[← Python 选择指南](README.md) · [Skill 入口](../../SKILL.md) · [可运行源码](../../examples/python/observer/main.py)
...
同步快照通知；异常 fail-fast 传播。dict 保留插入顺序，但不把这个顺序推广为所有语言的保证；长期订阅者必须自行解除注册。
...
```sh
python3 examples/python/observer/main.py
```

期望标准输出：`OK observer`。任何内嵌断言失败都应导致非零退出；Python 不要使用 `-O` 禁用断言，Swift 不要使用 `-Ounchecked`。

**当前源码状态：已通过运行与内嵌断言。** [完整验证记录](../../docs/VERIFICATION.md)。源码 SHA-256：`44093e66f834d5f82159e447a9b33715b91ae13b6cf0e4992440d528aa050d16`。
