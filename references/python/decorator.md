...
[← Python 选择指南](README.md) · [Skill 入口](../../SKILL.md) · [可运行源码](../../examples/python/decorator/main.py)
...
展示 GoF 对象包装，而不是 Python @decorator 语法。两层包装故意不满足交换律，以暴露组合顺序。
...
```sh
python3 examples/python/decorator/main.py
```

期望标准输出：`OK decorator`。任何内嵌断言失败都应导致非零退出；Python 不要使用 `-O` 禁用断言，Swift 不要使用 `-Ounchecked`。

**当前源码状态：已通过运行与内嵌断言。** [完整验证记录](../../docs/VERIFICATION.md)。源码 SHA-256：`5c245f24be33ac101f7f68a59ddff8542a525f90758c7259a5e5fe1a1cff37f3`。
