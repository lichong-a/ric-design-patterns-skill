# Python · 生成器（建造者）（Builder）

[← Python 选择指南](README.md) · [Skill 入口](../../SKILL.md) · [可运行源码](../../examples/python/builder/main.py)
...
现代单产品构建器变体；tuple 隔离章节容器，frozen dataclass 仅冻结字段绑定。成员为不可变字符串，因此本例快照不共享可变子对象。
...
```sh
python3 examples/python/builder/main.py
```

期望标准输出：`OK builder`。任何内嵌断言失败都应导致非零退出；Python 不要使用 `-O` 禁用断言，Swift 不要使用 `-Ounchecked`。

**当前源码状态：已通过运行与内嵌断言。** [完整验证记录](../../docs/VERIFICATION.md)。源码 SHA-256：`4430654f7293d84bf3e2f3e8aab0835f05c5476871bdff27d709fc596c5901a3`。
