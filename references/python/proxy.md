...
[← Python 选择指南](README.md) · [Skill 入口](../../SKILL.md) · [可运行源码](../../examples/python/proxy/main.py)
...
通过 Optional 主体演示单线程延迟加载。并发首次读取需要锁或其他原子初始化机制；本例没有这项保证。
...
```sh
python3 examples/python/proxy/main.py
```

期望标准输出：`OK proxy`。任何内嵌断言失败都应导致非零退出；Python 不要使用 `-O` 禁用断言，Swift 不要使用 `-Ounchecked`。

**当前源码状态：已通过运行与内嵌断言。** [完整验证记录](../../docs/VERIFICATION.md)。源码 SHA-256：`19f3293bbf991827847f1e0d4247dda43336d633228657e255b38a5ee9cb770e`。
