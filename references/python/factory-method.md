# Python · 工厂方法（Factory Method）

[← Python 选择指南](README.md) · [Skill 入口](../../SKILL.md) · [可运行源码](../../examples/python/factory-method/main.py)
...
ABC 保留经典创建者覆写结构；产品使用 Protocol，不要求显式继承。Protocol 注解不会自动做运行时参数校验。
...
```sh
python3 examples/python/factory-method/main.py
```

期望标准输出：`OK factory-method`。任何内嵌断言失败都应导致非零退出；Python 不要使用 `-O` 禁用断言，Swift 不要使用 `-Ounchecked`。

**当前源码状态：已通过运行与内嵌断言。** [完整验证记录](../../docs/VERIFICATION.md)。源码 SHA-256：`cb36d1e987afa3dfde0d13db2b19db5507a51a4d6c30ef8ee43c0a0909082534`。
