...
[← Python 选择指南](README.md) · [Skill 入口](../../SKILL.md) · [可运行源码](../../examples/python/strategy/main.py)
...
单方法算法族优先用 Callable 与纯函数，无需创建空策略类。上下文负责通用输入约束，具体算法负责自己的数学规则。
...
```sh
python3 examples/python/strategy/main.py
```

期望标准输出：`OK strategy`。任何内嵌断言失败都应导致非零退出；Python 不要使用 `-O` 禁用断言，Swift 不要使用 `-Ounchecked`。

**当前源码状态：已通过运行与内嵌断言。** [完整验证记录](../../docs/VERIFICATION.md)。源码 SHA-256：`7bf482b3758a6d3ae53e46a33c8d0661e4cbb992b228497c2fbdccc8cc729ea5`。
