...
[← Python 选择指南](README.md) · [Skill 入口](../../SKILL.md) · [可运行源码](../../examples/python/interpreter/main.py)
...
使用不可变 AST 和只读映射接口；未知变量保留 KeyError。这里只实现 AST 求值，不实现字符串解析，也不调用 eval。
...
```sh
python3 examples/python/interpreter/main.py
```

期望标准输出：`OK interpreter`。任何内嵌断言失败都应导致非零退出；Python 不要使用 `-O` 禁用断言，Swift 不要使用 `-Ounchecked`。

**当前源码状态：已通过运行与内嵌断言。** [完整验证记录](../../docs/VERIFICATION.md)。源码 SHA-256：`807821179360a871051492bc6ab7a8af8326d98375af6bd797dbba24ddc809ea`。
