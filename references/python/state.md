...
[← Python 选择指南](README.md) · [Skill 入口](../../SKILL.md) · [可运行源码](../../examples/python/state/main.py)
...
状态对象返回后继状态，转换规则不散落在 Gate 中。状态无实例可变字段，示例仍为每次转换构造独立对象以保持简单生命周期。
...
```sh
python3 examples/python/state/main.py
```

期望标准输出：`OK state`。任何内嵌断言失败都应导致非零退出；Python 不要使用 `-O` 禁用断言，Swift 不要使用 `-Ounchecked`。

**当前源码状态：已通过运行与内嵌断言。** [完整验证记录](../../docs/VERIFICATION.md)。源码 SHA-256：`c80cab89ad0006a0c10632d561e596a918610c59f5140c227688434e36d25ccc`。
