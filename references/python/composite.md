...
[← Python 选择指南](README.md) · [Skill 入口](../../SKILL.md) · [可运行源码](../../examples/python/composite/main.py)
...
使用只读孩子 tuple，叶子与容器都满足 Item 协议；本例只构造树，不支持客户端引入父子环。
...
```sh
python3 examples/python/composite/main.py
```

期望标准输出：`OK composite`。任何内嵌断言失败都应导致非零退出；Python 不要使用 `-O` 禁用断言，Swift 不要使用 `-Ounchecked`。

**当前源码状态：已通过运行与内嵌断言。** [完整验证记录](../../docs/VERIFICATION.md)。源码 SHA-256：`4ca1a25200b7c7c66c2f18b92416928cda1aa182bb9f5d5e74d9189fbf8a3552`。
