...
[← Python 选择指南](README.md) · [Skill 入口](../../SKILL.md) · [可运行源码](../../examples/python/command/main.py)
...
命令显式维护 new/done/undone 生命周期。本例采用快照恢复且不允许命令间交错修改；生产中需版本检查以避免覆盖其他操作。
...
```sh
python3 examples/python/command/main.py
```

期望标准输出：`OK command`。任何内嵌断言失败都应导致非零退出；Python 不要使用 `-O` 禁用断言，Swift 不要使用 `-Ounchecked`。

**当前源码状态：已通过运行与内嵌断言。** [完整验证记录](../../docs/VERIFICATION.md)。源码 SHA-256：`8b6bba230dfde9a42c8f1e84ea687ce23e4d14cfb4e2abd70809068e148fdb19`。
