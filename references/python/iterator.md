...
[← Python 选择指南](README.md) · [Skill 入口](../../SKILL.md) · [可运行源码](../../examples/python/iterator/main.py)
...
实现 __iter__ 接入 for/list 等原生工具；每次创建独立 tuple iterator，避免集合自己兼任单个游标。
...
```sh
python3 examples/python/iterator/main.py
```

期望标准输出：`OK iterator`。任何内嵌断言失败都应导致非零退出；Python 不要使用 `-O` 禁用断言，Swift 不要使用 `-Ounchecked`。

**当前源码状态：已通过运行与内嵌断言。** [完整验证记录](../../docs/VERIFICATION.md)。源码 SHA-256：`6879097a9fe49d821fae8123c320584f6955982189ca96a7fe0581828474a8de`。
...
- [参考站语言示例抽样](https://refactoringguru.cn/design-patterns/iterator/python/example)
