...
[← Python 选择指南](README.md) · [Skill 入口](../../SKILL.md) · [可运行源码](../../examples/python/flyweight/main.py)
...
Style 为不可变值，池共享实例；字形位置是外在状态。dict 的多步检查/插入不应靠 GIL 假设成并发契约。
...
```sh
python3 examples/python/flyweight/main.py
```

期望标准输出：`OK flyweight`。任何内嵌断言失败都应导致非零退出；Python 不要使用 `-O` 禁用断言，Swift 不要使用 `-Ounchecked`。

**当前源码状态：已通过运行与内嵌断言。** [完整验证记录](../../docs/VERIFICATION.md)。源码 SHA-256：`2e05b145c7b4ed5bd3c6c1699de7cbff6efc124f7612fd1950d77e2e45a22ce0`。
