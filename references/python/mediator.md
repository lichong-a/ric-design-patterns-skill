...
[← Python 选择指南](README.md) · [Skill 入口](../../SKILL.md) · [可运行源码](../../examples/python/mediator/main.py)
...
同事通过回调向 Dialog 报告事件，不直接引用彼此。绑定方法会保留 self；扩展到长寿命订阅时需要明确解除回调的时机。
...
```sh
python3 examples/python/mediator/main.py
```

期望标准输出：`OK mediator`。任何内嵌断言失败都应导致非零退出；Python 不要使用 `-O` 禁用断言，Swift 不要使用 `-Ounchecked`。

**当前源码状态：已通过运行与内嵌断言。** [完整验证记录](../../docs/VERIFICATION.md)。源码 SHA-256：`1355f04aebd8d4622eac83ad3c75e4d0d91d14eda47fa04b6b6a9cb243f562b4`。
