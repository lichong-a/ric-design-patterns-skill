# JavaScript · 装饰（Decorator）

[← JavaScript 选择指南](README.md) · [Skill 入口](../../SKILL.md) · [可运行源码](../../examples/javascript/decorator/main.mjs)
...
|---|---|
| Component | Text：read 操作（未显式声明的接口角色由方法约定承担） |
...
```sh
node examples/javascript/decorator/main.mjs
```

期望标准输出：`OK decorator`。任何内嵌断言失败都应导致非零退出；Python 不要使用 `-O` 禁用断言，Swift 不要使用 `-Ounchecked`。

**当前源码状态：已通过运行与内嵌断言。** [完整验证记录](../../docs/VERIFICATION.md)。源码 SHA-256：`c816e02baa1098fd4d8404a417b3e5f985d525eaa71386ced95a6321bac60334`。
