# JavaScript · 生成器（建造者）（Builder）

[← JavaScript 选择指南](README.md) · [Skill 入口](../../SKILL.md) · [可运行源码](../../examples/javascript/builder/main.mjs)
...
|---|---|
| Builder | ReportBuilder：积累标题和章节并 build（未显式声明的接口角色由方法约定承担） |
| Product | Report：完成后的报告快照（未显式声明的接口角色由方法约定承担） |
| Director | 可选的配方/调用代码：决定组装次序，本例不另设类（未显式声明的接口角色由方法约定承担） |
...
ES2022 原创示例，与 TypeScript 版本同源并经过类型擦除；用鸭子类型、类/闭包和显式运行时检查表达契约。# 私有字段和两级 Object.freeze 保留实际运行时约束，freeze 并不是递归冻结。
...
```sh
node examples/javascript/builder/main.mjs
```

期望标准输出：`OK builder`。任何内嵌断言失败都应导致非零退出；Python 不要使用 `-O` 禁用断言，Swift 不要使用 `-Ounchecked`。

**当前源码状态：已通过运行与内嵌断言。** [完整验证记录](../../docs/VERIFICATION.md)。源码 SHA-256：`a100f6ca476f97f0621ccf5198de343a72ffd077828965d44ca2623c0c00961e`。
