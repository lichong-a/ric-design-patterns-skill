# JavaScript · 模板方法（Template Method）

[← JavaScript 选择指南](README.md) · [Skill 入口](../../SKILL.md) · [可运行源码](../../examples/javascript/template-method/main.mjs)
...
|---|---|
| AbstractClass | Importer：run 组织稳定流程（未显式声明的接口角色由方法约定承担） |
| PrimitiveOperation | parse：受控扩展步骤（未显式声明的接口角色由方法约定承担） |
| ConcreteClass | UpperImporter：实现具体解析（未显式声明的接口角色由方法约定承担） |
...
ES2022 原创示例，与 TypeScript 版本同源并经过类型擦除；用鸭子类型、类/闭包和显式运行时检查表达契约。JavaScript 没有 abstract/final；子类必须实现 parse，不覆写 run 是约定。
...
```sh
node examples/javascript/template-method/main.mjs
```

期望标准输出：`OK template-method`。任何内嵌断言失败都应导致非零退出；Python 不要使用 `-O` 禁用断言，Swift 不要使用 `-Ounchecked`。

**当前源码状态：已通过运行与内嵌断言。** [完整验证记录](../../docs/VERIFICATION.md)。源码 SHA-256：`fc02391aa657fd0498194814e9a4eaed8f4646d2947f5c58f4ca984a7824777b`。
